...
//TODO: Reporting(finding x of yy track, xx tracks founded...)
vkSearchWindow::vkSearchWindow(const wxString& title, searchOptions options) : wxFrame(NULL, wxID_ANY, title){	
	m_tracks = NULL;
	m_popupMenu = NULL;
	m_apiId = options.api_id;
	m_secret = options.secret;
	m_viewerId = options.viewer_id;
	...
	m_searchPanel = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxMINIMIZE_BOX | wxMAXIMIZE_BOX | wxTHICK_FRAME, _("search panel"));
	m_queryTextBox = new wxTextCtrl(m_searchPanel, QUERY_EDIT);
	m_searchButton = new wxButton(m_searchPanel, SEARCH_BUTTON, _("search"));
	m_searchResult = new wxListCtrl(m_searchPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLC_REPORT);	
	m_addAll = new wxButton(m_searchPanel, ADDALL_BUTTON, "Add all");
	m_addSelected = new wxButton(m_searchPanel, ADDSELECTED_BUTTON, "Add selected");
	m_deleteSelected = new wxButton(m_searchPanel, DELETESELECTED_BUTTON, "Delete selected");
	m_keepPrevious = new wxCheckBox(m_searchPanel, wxID_ANY, _("Keep previous search result"));

	m_searchResult->InsertColumn(0, _("Artist"));
	m_searchResult->InsertColumn(1, _("Title"));
	m_searchResult->InsertColumn(2, _("Duration"), wxLIST_FORMAT_RIGHT);
	...
	CreateStatusBar();
	SetStatusText(_("vk.com audio search"));
	...
	if(string.Len()){
		if(CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)){
			if(CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)){
				if(CryptHashData(hHash, (const BYTE*)string.c_str(), string.Len(), 0))
				{
					if(CryptGetHashParam(hHash, HP_HASHVAL, rgbHash, &cbHash, 0)){
						result = new wxString();
						result->Printf(	"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", 
...
wxString* vkSearchWindow::BuildQueryString(wxString query){
	wxString* result = NULL;
	if(query.Len()){
		wxString signature;
		wxString* hash;

		signature.Printf("%sapi_id=%scount=200method=audio.searchq=%stest_mode=1%s", m_viewerId, m_apiId, query, m_secret);
		hash = StringHash(signature);
		if(hash){
			result = new wxString();

			result->Printf(	"http://api.vkontakte.ru/api.php?api_id=%s&count=200&method=audio.search&sig=%s&test_mode=1&q=%s", 
							m_apiId, hash->c_str(), query);
		}
	}
	return result;
}
bool vkSearchWindow::BuildTrackList(wxString *queryString){
	bool success = false;

	if(queryString && queryString->Len()){
		wxURL url(queryString->c_str());
...
DWORD __stdcall vkSearchWindow::SearchThread(){
	wxString query = m_queryTextBox->GetValue().ToUTF8().data();
	wxString *queryString = NULL;
	bool success = false;

	m_searchButton->Disable();
	SetStatusText(_("working..."));
	...
	if(query.StartsWith(_("tag:"))){
		query.Replace(_("tag:"), wxEmptyString);
		success = LastFmTagSearch(query);
	} else if(query.StartsWith(_("vk:"))){
		query.Replace(_("vk:"), wxEmptyString);
	...
	} else if(query.StartsWith(_("radio:"))){
		query.Replace(_("radio:"), wxEmptyString);
...
Audio* vkSearchWindow::VkSearchTrack(wxString artistName, wxString trackName){
	...
	if(artistName.Len() && trackName.Len()){
		query.Printf("%s - %s", artistName, trackName);
		queryString = BuildQueryString(query);
		if(queryString){
			wxURL url(queryString->c_str());
...
void vkSearchWindow::AddTrackToResultList(Audio *track){
	...
	newTrack.SetText(wxString(conv.cMB2WC(track->artist.c_str())));
	newTrack.SetId(m_searchResult->GetItemCount());
	newTrack.SetData((void*)track->url.c_str());
	duration.Printf(_("%2d:%02d"), track->duration / 60, track->duration % 60);						
	index = m_searchResult->InsertItem(newTrack);
	m_searchResult->SetItem(index, 1, wxString(conv.cMB2WC(track->title.c_str())));
...
bool vkSearchWindow::LastFmArtistSearch( wxString artist, int maxCount /*= 50*/ )
{
	...
	requestUrl.Printf("http://ws.audioscrobbler.com/1.0/artist/%s/toptracks.xml",  escapedArtist);
	...
	if(url.IsOk()){
		...
		if(toptracks){
			...
			while(node && !m_vkRequestError){
				if(node->GetName() == _("track")){
					wxXmlNode *track = node->GetChildren();

					if(track->GetName() == _("name")){
			...
			SetStatusText("last.fm request failed. try vk:" + artist);
		}
	}
	return success;
}
bool vkSearchWindow::LastFmTagSearch( wxString tag )
{
	...
	requestUrl.Printf("http://ws.audioscrobbler.com/1.0/tag/%s/toptracks.xml", escapedTag);
	requestUri.Create(requestUrl);
	wxURL url(requestUri);
	if(url.IsOk()){
		wxInputStream *toptracks = url.GetInputStream();
		if(toptracks){
			...
			while(track && !m_vkRequestError){
				if(track->GetName() == _("track")){
					wxXmlNode *artist = track->GetChildren();
					wxString trackName = track->GetAttribute(_("name"), "");
					wxString artistName = artist->GetAttribute(_("name"), "");
			...
			SetStatusText("last.fm request failed...");
		}
	}
	return success;
}
bool vkSearchWindow::LastFmArtistRadio( wxString artist )
{
	...
	AtlEscapeUrl(artist.c_str(), escapedArtist.get(), &chEscaped, INTERNET_MAX_URL_LENGTH);
	requestUrl.Printf("http://ws.audioscrobbler.com/1.0/artist/%s/similar.xml",  escapedArtist);
	requestUri.Create(requestUrl);
	wxURL url(requestUri);
	if(url.IsOk()){
		...
		if(similar){
			...
			while(node && !m_vkRequestError){
				if(node->GetName() == _("artist")){
					wxXmlNode *similarArtist = node->GetChildren();

					if(similarArtist->GetName() == _("name")){
...
void vkSearchWindow::AddSelected(){
	if(m_addTracksFn){
		...
		while((item = m_searchResult->GetNextItem(item, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED)) != wxNOT_FOUND){
			tracks.push_back((char*)m_searchResult->GetItemData(item));
		}
...
void vkSearchWindow::OnAddAllClick(wxCommandEvent& evt){
	if(m_addTracksFn){
		...
		for(long i = 0; i < m_searchResult->GetItemCount(); i++){
			tracks.push_back((char*)m_searchResult->GetItemData(i));
		}
		m_addTracksFn(tracks);
		if(m_closeAfterAdd)
		{
...
void vkSearchWindow::OnContextMenu(wxContextMenuEvent& evt){		
	if(!m_popupMenu){
		...
		m_popupMenu->Append(ADDSELECTED_MENU, "Add selected tracks to playlist");
		m_popupMenu->Append(DELETESELECTED_MENU, "Delete selected tracks");
		m_popupMenu->Append(COPY_URL_MENU, "Copy url(s) to clipboard");
